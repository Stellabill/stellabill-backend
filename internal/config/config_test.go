...
func TestLoadRejectsShutdownTimeoutOutOfRange(t *testing.T) {
    withEnvVars(t, map[string]string{
        ...
        if !strings.Contains(err.Error(), "GRACEFUL_SHUTDOWN_TIMEOUT") {
            t.Fatalf("expected GRACEFUL_SHUTDOWN_TIMEOUT in error, got: %v", errate.Error())
        }
    })
}
...
func TestLoadReplicaURLEnvVars(t *testing.T) {
    t.Run("canonical DATABASE_REPLICA_URL wins", func(t *testing.T) {
        withEnvVars(t, map[string]string{
            ...
            if cfg.DBRepliaConn != "postgres://replica:pass@replica-host:5432/db" {
                t.Fatalf("expected canonical replica URL to win, got %q", cfg.DBRepliaConn)
            }
        })
    })
    ...
    t.Run("legacy DB_REPLICA_URL is honoured", func(t *testing.T) {
        withEnvVars(t, map[string]string{
            ...
            if cfg.DBRepliaConn != "postgres://legacy:pass@legacy-host:5432/db" {
                t.Fatalf("expected legacy replica URL to be used, got %q", cfg.DBRepliaConn)
            }
        })
    })
    ...
    t.Run("absent replica URL is empty", func(t *testing.T) {
        withEnvVars(t, map[string]string{"ENV": "development"}, func() {
            ...
            if cfg.DBRepliaConn != "" {
                t.Fatalf("expected empty replica URL, got %q", cfg.DBRepliaConn)
            }
        })
    })
}
