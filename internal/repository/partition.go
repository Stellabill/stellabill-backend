...
func isTablePartitioned(ctx context.Context, db *sql.DB, tableName string) (bool, error) {
	...
	query := `SELECT EXISTS (SELECT 1 FROM pg_partitioned_table WHERE partrelid = $1::regclass)`
...
func partitionExists(ctx context.Context, db *sql.DB, partitionName string) (bool, error) {
	var exists bool
	query := `SELECT EXISTS (SELECT 1 FROM pg_class WHERE relname = $1)`
	err := db.QueryRowContext(ctx, query, partitionName).Scan(&exists)
	if err != nil {
		return false, fmt.Errorf("check partition %s existence: %w", partitionName, err)
	}
	return exists, nil
}
